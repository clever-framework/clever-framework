...
public class AppSecurityRest extends AppBaseCrudController<BizNewsDomainService, BizNews> {
    ...
    public ResponseBodyWrap secured() {
        return ResponseBodyWrap.builder().content("secured").build();
    }

    // @PreAuthorize("hasAnyRole('normal')")
    @RequestMapping("/normal")
    public ResponseBodyWrap normal() {
        return ResponseBodyWrap.builder().content("normal").build();
    }

    // @PreAuthorize("hasAnyAuthority('admin')")
    @RequestMapping("/admin")
    public ResponseBodyWrap admin() {
        return ResponseBodyWrap.builder().content("admin").build();
    }


    // @PreAuthorize("hasAnyAuthority('normal','admin')")
    @RequestMapping("/all")
    public ResponseBodyWrap all() {
        return ResponseBodyWrap.builder().content("all").build();
    }
